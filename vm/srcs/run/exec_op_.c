...
int		exec_st(void *core, void *pro)
{
	...
	{
		insert_in_color(cr->mem_c, (pr->pc + get_n_arg(cr, pr, 2))
			% MEM_SIZE, pr->player->color * 16 + 2, 4);
		insert_in_reg(cr->mem, ((unsigned short int)(pr->pc +
			((short int)get_n_arg(cr, pr, 2) % IDX_MOD))
...
int		exec_sti(void *core, void *pro)
{
	...
	pr = (t_process*)pro;
	res = get_n_arg(cr, pr, 1);
	add = real_int(get_n_arg(cr, pr, 2) % MEM_SIZE) +
	real_int((get_n_arg(cr, pr, 3) % MEM_SIZE));
